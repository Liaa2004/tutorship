...
export default function StudentDashboard() {
  ...
  //     if (category === "certificate") {
  //       const newActivityId = `act-${Date.now()}`; // Generate a new ID for the activity
  
  //       const newActivity = {
  //         id: newActivityId, // Use the same ID for the activity
  //         studentId: userId,
  ...
  //       await api.post("http://localhost:4000/save-activity", newActivity);
  //     }
  ...
  const handleApplicationRequest = async (e) => {
    ...
    try {
      ...
      if (category === "certificate") {
        ...
        // This will save both the activity and create the linked request
        await api.post("http://localhost:4000/save-activity", newActivity);
      } else {
        // Handle non-certificate requests (scholarship, etc.)
        const newRequest = {
          id: `req-${Date.now()}`,
          category: category,
          status: "Pending",
        ...
        const updatedClass = {
          ...
          scholarshipApplications: [
            ...classData.scholarshipApplications,
            ...(category === "scholarship" ? [newApplication] : []),
          ],
        };
  ...
  const calculateTotalPoints = () => {
    // Only calculate points from approved certificate requests in the student's record
    return student?.requests
      ?.filter(req => 
        req.category === "certificate" && 
        req.status === "Approved"
      )
              ...
              {category === "certificate" && (
                <>
                  ...
                  <select
                    className="w-full p-2 border rounded"
                    value={level}
                    onChange={(e) => setLevel(e.target.value)} style={{
                      backgroundColor: "var(--bg-secondary)",
                      borderColor: "var(--bg-tertiary)",
                      color: "var(--text-primary)",
                    }}
