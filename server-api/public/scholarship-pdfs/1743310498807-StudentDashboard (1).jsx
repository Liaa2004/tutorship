...
export default function StudentDashboard() {
  ...
  const fetchStudentData = async () => {
    try {
      const response = await api.get(`classes/${classId}?timestamp=${Date.now()}`); // Cache busting
      ...
      // Filter out any null/undefined requests and ensure proper fields
      const validRequests = (studentDetails.requests || []).filter(Boolean).map(request => ({
        ...request,
        activityType: request.activityType || "",
        level: request.level || "",
        maxPoints: request.maxPoints || 0,
        awardedPoints: request.awardedPoints || 0,
  ...
  const fetchActivityPoints = async () => {
    try {
      const response = await api.get(`/activityPoints?studentId=${userId}`);
      setActivityPoints(response.data);
    } catch (error) {
      console.error("Error fetching activity points:", error);
      setMessage("Failed to load activity points. Please try again later.");
    }
  };
  ...
  useEffect(() => {
    ...
    fetchStudentData();
    fetchActivityPoints(); // Fetch activity points on component mount
  }, [navigate]);
    ...
    try {
      ...
      const updatedStudents = classData.students.map((student) => {
        if (student.id === userId) {
          return {
            ...student,
            requests: [...(student.requests || []).filter(Boolean), newRequest],
          };
        }
        return student;
      });
      ...
      const updatedClass = {
        ...
        scholarshipApplications: [
          ...
          ...(category === "scholarship" ? [newApplication] : []), // Only add if it's a scholarship
        ],
      };
      ...
      setStudent(prev => ({
        ...prev,
        requests: [...(prev?.requests || []).filter(Boolean), newRequest]
      }));
  ...
  //       // Save the activity to the activityPoints
  //       await api.post("/save-activity", newActivity);
  //     }
  ...
  //     setApplications((prevApplications) => [
  //       ...prevApplications,
  //       newRequest, // This should be a request, not an application
  //     ]);
  ...
  //         return {
  //           ...student,
  //           requests: [...(student.requests || []), newRequest], // Add to requests
  //         };
  //       }
  //       return student;
  //     });
  ...
  //       ...classData,
  //       students: updatedStudents,
  //       // Do not add to scholarshipApplications unless it's a scholarship
  //       scholarshipApplications: [
  //         ...classData.scholarshipApplications,
  //         ...(category === "scholarship" ? [newRequest] : []), // Only add if it's a scholarship
  //       ],
  //     };
  ...
  //     setStudent(prev => ({
  //       ...prev,
  //       requests: [...(prev?.requests || []), newRequest] // Update student requests
  //     }));
  //     console.log("Student request added");
  //     // Reset form
  //     setCategory("");
  ...
  //   if (!student?.requests) return {};
    
  //   // Filter out any null/undefined requests first
  //   const validRequests = (student.requests || []).filter(Boolean);
    
  //   return validRequests.reduce((acc, request) => {
  //     if (!request.category) return acc;
      
  //     if (!acc[request.category]) {
  //       acc[request.category] = [];
  //     }
      
  //     acc[request.category].push({
  //       ...request,
  //       activityType: request.activityType || "-",
  //       level: request.level || "-",
  //       maxPoints: request.maxPoints || 0,
  ...
  const calculateTotalPoints = () => {
    // Points from approved activity points
    const activityPointsTotal = activityPoints
      .filter((activity) => activity.status === "Approved")
      .reduce((total, activity) => total + (activity.pointsAwarded || 0), 0);

    const requestPointsTotal =
      student?.requests
        ?.filter(
          (req) => req.category === "certificate" && req.status === "Approved"
        )
        .reduce((total, req) => total + (req.awardedPoints || 0), 0) || 0;

    return activityPointsTotal + requestPointsTotal;
  };
  return (
    <div
                    ...
                    ([category, requests]) => (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {category === "certificate"
                              ? `Activities/Certificates (Total Points: ${calculateTotalPoints()})`
                              : category}
                          </span>
                          <span>
